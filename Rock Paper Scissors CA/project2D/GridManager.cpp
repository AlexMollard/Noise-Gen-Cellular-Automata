...
void GridManager::FinalTouches()
{
	for (int x = 0; x < _CellTotal; x++)
	{
		for (int y = 0; y < _CellTotal; y++)
		{
			...
			int _randomNummber = rand() % 30;

			// Food Chain	1: _Tree, 2: _Shallow, 3: _DeepWater, 4: _Sand, 5: _Grass
			if (_Cells[x][y].GetType() == '4' && _DeepWaterNeighbours > 5)
			{
				_Cells[x][y].SetChangeDeepWaters(true);
			}
			
			if (_Cells[x][y].GetType() == '3' && _GrassNeighbours > 1)
			{
				_Cells[x][y].SetChangeSand(true);
			}

			if (_Cells[x][y].GetType() == '2' && _DeepWaterNeighbours > 4)
			{
				_Cells[x][y].SetChangeDeepWaters(true);
			}

			if (_Cells[x][y].GetType() == '3' && _SandNeighbours > 0)
			{
				_Cells[x][y].SetChangeShallow(true);
			}

			if (_Cells[x][y].GetType() == '1' && _ShallowNeighbours > 1)
			{
				_Cells[x][y].SetChangeSand(true);
			}

			if (_Cells[x][y].GetType() == '1' && _DeepWaterNeighbours > 1)
			{
				_Cells[x][y].SetChangeDeepWaters(true);
			}

			if (_Cells[x][y].GetType() == '5' && _ShallowNeighbours > 2)
			{
				_Cells[x][y].SetChangeDeepWaters(true);
			}


			if (_Cells[x][y].GetType() == '4' && _GrassNeighbours > 3 && _ShallowNeighbours < 2)
			{
				_Cells[x][y].SetChangeGrass(true);
			}

			if (_Cells[x][y].GetType() == '5' && _TreeNeighbours > 2)
			{
				_Cells[x][y].SetChangeTree(true);
			}
			
			if (_Cells[x][y].GetType() == '5' && _GrassNeighbours > 5 && _randomNummber == 1 && _Round == 101)
			{
				_Cells[x][y].SetChangeTree(true);
			}
		}
